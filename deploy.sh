...
# 拍立得相机 - Vercel 部署脚本
# 此脚本帮助你将项目推送到新的 GitHub 仓库并部署到 Vercel
...
if [ -f ".env" ] || [ -f ".env.local" ]; then
    echo "⚠️  警告：检测到 .env 或 .env.local 文件"
    echo "   这些文件不应提交到 Git，已在 .gitignore 中排除"
fi
...
read -p "仓库名称 (instax-camera-web): " REPO_NAME
REPO_NAME=${REPO_NAME:-instax-camera-web}
...
echo ""
echo "💾 提交更改..."
git commit -m "feat: 初始化拍立得相机应用，支持 Vercel 部署" || echo "ℹ️  没有新的更改需要提交"
